...
function start_kasmvnc() {
	...
	local ssl_options="-sslOnly 1"
	if [ $KASM_VNC_SSL = "0" ]; then
		ssl_options="-sslOnly 0"
	fi

	local auth_options=""
	if [ $KASM_NO_AUTH = "1" ]; then
		auth_options="-disableBasicAuth"
	fi

	if [[ "${BUILD_ARCH}" =~ ^aarch64$ ]] && [[ -f /lib/aarch64-linux-gnu/libgcc_s.so.1 ]]; then
		LD_PRELOAD=/lib/aarch64-linux-gnu/libgcc_s.so.1 vncserver $DISPLAY $KASMVNC_HW3D -drinode $DRINODE -depth $VNC_COL_DEPTH -geometry $VNC_RESOLUTION -websocketPort $NO_VNC_PORT -httpd ${KASM_VNC_PATH}/www $ssl_options $auth_options -FrameRate=$MAX_FRAME_RATE -interface 0.0.0.0 -BlacklistThreshold=0 -FreeKeyMappings $VNCOPTIONS $KASM_SVC_SEND_CUT_TEXT $KASM_SVC_ACCEPT_CUT_TEXT
	else
		vncserver $DISPLAY $KASMVNC_HW3D -drinode $DRINODE -depth $VNC_COL_DEPTH -geometry $VNC_RESOLUTION -websocketPort $NO_VNC_PORT -httpd ${KASM_VNC_PATH}/www $ssl_options $auth_options -FrameRate=$MAX_FRAME_RATE -interface 0.0.0.0 -BlacklistThreshold=0 -FreeKeyMappings $VNCOPTIONS $KASM_SVC_SEND_CUT_TEXT $KASM_SVC_ACCEPT_CUT_TEXT
	fi
...
function start_upload() {
	if [[ ${KASM_SVC_UPLOADS:-1} == 1 ]]; then
		local ssl_options="--ssl"
		if [ $KASM_VNC_SSL = "0" ]; then
			ssl_options=""
		fi

		log 'Starting upload server'
		$STARTUPDIR/upload_server/kasm_upload_server $ssl_options --auth-token "kasm_user:$VNC_PW" --port 4902 --upload_dir ${HOME}/Uploads &
...
function wait_for_network_devices() {
	# if network devices are not bridged, just return
	if [ "$KASM_NO_VETH" = '1' ]; then
		return
	fi
	while true; do
